...
const inter = ref<number>()
function initWs() {
  const comet_url = 'wss://mr-crm.erpgj.com/websocket'
  const ws = new WebSocket(comet_url)
  ...
  ws.onopen = function () {
    ...
    ws.send(JSON.stringify({ authorization: useUserStore().token, type: 'bind' }))
  }
  ws.onmessage = function (res) {
    const { data: wsMsgData, msg } = JSON.parse(res.data)

    console.log(wsMsgData, msg)

    if (msg === '登出') {
      ElMessage({
        message: '当前异地登陆已退出通知能力',
        type: 'error',
      })
    }
    if (wsMsgData && wsMsgData.crm_csr_track_id) {
      ElNotification({
        title: wsMsgData.csr_name,
        message: `下次接待跟踪：${wsMsgData.next_follow_time}`,
        type: 'warning',
      })
    }
  }
