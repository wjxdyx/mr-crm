...
// 客户通知列表
const userNoticeList = ref<userNoticeItem[]>([])
apiCsr.waitTrackCsr({ index: 1, limit: 20 }).then((r) => {
  userNoticeList.value = r.data
  if (userNoticeList.value.length > 0) {
    ElMessage.warning('今日有待回访需处理')
  }
})

function clickMoney(_command: string) {
  router.push({
    name: 'csrFollowUp',
  })
}
  ...
  <div class="tools">
    <div class="buttons">
      ...
      <span class="item">
        ...
        <el-dropdown :hide-on-click="false" @command="clickMoney">
          <span class="el-dropdown-link">
            <el-badge :is-dot="userNoticeList.length > 0" class="item"><el-icon> <svg-icon
              v-show="settingsStore.settings.app.colorScheme === 'light'" name="ep:bell"
            /></el-icon></el-badge>
          </span>
          <template #dropdown>
            <el-dropdown-menu>

              <el-dropdown-item v-for="item, index in userNoticeList" :key="index">
                [ {{ item.csr_name }}]需要回访
                <span style="font-size: 9px;color: #797979;padding-left: 10px;">{{ item?.next_follow_time }}</span>
              </el-dropdown-item>
              <el-dropdown-item v-if="userNoticeList.length === 0" command="csrFollowUp">当日暂无回访</el-dropdown-item>
              <!-- <el-dropdown-item command="csrFollowUp">查看更多</el-dropdown-item> -->
